...
/*****************************************************************************
 * File name:   src/drivers/pfdc/pfdc-img-xdf.c                              *
...
static
int xdf_load_sector (FILE *fp, pfdc_img_t *img, const xdf_map_t *map, unsigned long ofs, unsigned c)
{
	unsigned   size;
	pfdc_sct_t *sct;
	...
	sct = pfdc_sct_new (c, map->h, map->s, size);
	...
	if (pfdc_read_ofs (fp, map->ofs + ofs, sct->data, size)) {
		return (1);
	}
...
static
int xdf_load_fp (FILE *fp, pfdc_img_t *img, const xdf_format_t *fmt)
{
	unsigned      i, j;
	unsigned long ofs;
...
static
int xdf_save_sector (FILE *fp, const pfdc_img_t *img, const xdf_map_t *map, unsigned long ofs, unsigned c)
{
	unsigned         size;
	...
	sct = pfdc_img_get_sector ((pfdc_img_t *) img, c, map->h, map->s, 0);
	...
	if (pfdc_write_ofs (fp, map->ofs + ofs, sct->data, size)) {
		return (1);
	}
...
static
int xdf_save_fp (FILE *fp, const pfdc_img_t *img, const xdf_format_t *fmt)
{
	unsigned      i, j;
	unsigned long ofs;
