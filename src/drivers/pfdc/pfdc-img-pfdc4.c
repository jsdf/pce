...
static
void pfdc4_sct_set_flags (pfdc_sct_t *sct, unsigned flg)
{
...
static
int pfdc4_load_sect (FILE *fp, pfdc_img_t *img, pfdc_sct_t **last, unsigned long size, unsigned long crc)
{
	...
	pc = pfdc_get_uint16_be (buf, 0);
	ph = pfdc_get_uint16_be (buf, 2);
	lc = pfdc_get_uint16_be (buf, 4);
	lh = pfdc_get_uint16_be (buf, 6);
	ls = pfdc_get_uint16_be (buf, 8);
	n = pfdc_get_uint16_be (buf, 10);
	ex = buf[12];
	fl = pfdc_get_uint16_be (buf, 14);
	en = pfdc_get_uint16_be (buf, 16);

	sct = pfdc_sct_new (lc, lh, ls, n);
	...
	pfdc_sct_set_encoding (sct, en);

	switch (en & PFDC_ENC_MASK) {
	case PFDC_ENC_FM:
	case PFDC_ENC_MFM:
		pfdc_sct_set_mfm_size (sct, ex);
		break;

	case PFDC_ENC_GCR:
		pfdc_sct_set_gcr_format (sct, ex);
		break;
	}
	...
	if (fl & PFDC4_FLAG_ALTERNATE) {
		...
		if (pfdc_img_add_sector (img, sct, pc, ph)) {
			pfdc_sct_del (sct);
			return (1);
		}
	}
...
static
int pfdc4_load_data (FILE *fp, pfdc_img_t *img, pfdc_sct_t *sct, unsigned long size, unsigned long crc)
{
	unsigned cnt;
...
static
int pfdc4_load_tags (FILE *fp, pfdc_sct_t *last, unsigned long size, unsigned long crc)
{
	unsigned      cnt;
	unsigned char buf[256];
...
static
int pfdc4_load_text (FILE *fp, pfdc_img_t *img, unsigned long size, unsigned long crc)
{
	int           r;
	unsigned long i, n;
	unsigned char *buf;
...
static
int pfdc4_load_end (FILE *fp, pfdc_img_t *img, unsigned long size, unsigned long crc)
{
	if (pfdc4_skip_chunk (fp, size, crc)) {
		return (1);
	}
...
int pfdc4_load_fp (FILE *fp, pfdc_img_t *img, unsigned long id, unsigned long sz)
{
	...
	while (1) {
		...
		ckid = pfdc_get_uint32_be (buf, 0);
		size = pfdc_get_uint32_be (buf, 4);
...
int pfdc4_save_chunk (FILE *fp, unsigned ckid, unsigned size, const void *data)
{
	...
	pfdc_set_uint32_be (buf, 0, ckid);
	pfdc_set_uint32_be (buf, 4, size);
...
int pfdc4_save_header (FILE *fp)
{
	...
	pfdc_set_uint16_be (buf, 0, 4);
	pfdc_set_uint16_be (buf, 2, 0);
...
static
unsigned long pfdc4_sct_get_flags (const pfdc_sct_t *sct)
{
	unsigned long f;
...
static
int pfdc4_save_sector (FILE *fp, const pfdc_sct_t *sct, unsigned c, unsigned h, int alt)
{
	unsigned      i;
	unsigned      flg, cnt;
	unsigned char buf[256];
	...
	pfdc_set_uint16_be (buf, 0, c);
	pfdc_set_uint16_be (buf, 2, h);
	pfdc_set_uint16_be (buf, 4, sct->c);
	pfdc_set_uint16_be (buf, 6, sct->h);
	pfdc_set_uint16_be (buf, 8, sct->s);
	pfdc_set_uint16_be (buf, 10, sct->n);
	pfdc_set_uint16_be (buf, 12, 0);
	pfdc_set_uint16_be (buf, 14, flg);
	pfdc_set_uint16_be (buf, 16, sct->encoding);

	if (flg & PFDC4_FLAG_COMPRESSED) {
		buf[13] = sct->data[0];
	}

	switch (sct->encoding & PFDC_ENC_MASK) {
	case PFDC_ENC_FM:
	case PFDC_ENC_MFM:
		buf[12] = pfdc_sct_get_mfm_size (sct);
		break;

	case PFDC_ENC_GCR:
		buf[12] = pfdc_sct_get_gcr_format (sct);
		break;
	}
...
static
int pfdc4_save_alternates (FILE *fp, const pfdc_sct_t *sct, unsigned c, unsigned h)
{
...
static
int pfdc4_save_text (FILE *fp, const pfdc_img_t *img)
{
	int           r;
	unsigned long crc;
	...
	pfdc_set_uint32_be (buf, 0, PFDC4_CHUNK_TEXT);
	pfdc_set_uint32_be (buf, 4, img->comment_size + 2);
